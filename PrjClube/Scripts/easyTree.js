...
(function ($) {
    $.fn.EasyTree = function (options) {
        var defaults = {
            ...
            addable: false,
            i18n: {
                deleteNull: 'Selecione um nó para apagar',
                deleteConfirmation: 'Excluir este nó?',
                confirmButtonLabel: 'OK',
                editNull: 'Selecione um nó para editar',
                editMultiple: 'Apenas um nó pode ser editado por vez',
                addMultiple: 'Selecione um nó para adicionar um novo nó',
                expandTip: 'expandir',
                selectTip: 'selcionar',
                unselectTip: 'desselecionar',
                editTip: 'editar',
                addTip: 'adicionar',
                deleteTip: 'excluir',
        ...
        this.each(function () {
            ...
            $.each($(easyTree).find('ul > li'), function() {
                ...
                if($(this).is('li:has(ul)')) {
                    ...
                    var visiveis = $(this).find(' > ul > li:visible').length;
                    var link = $(this).find('a').attr('href');

                    $(children).remove();
                    text = $(this).text();
                    $(this).html('<span><span class="glyphicon"></span> </span> <span><a href="javascript: void(0);"></a> </span>');

                   

                    if (visiveis > 0) {
                        //$(this).find(' > span > span').addClass('glyphicon-folder-open').attr('title', options.i18n.collapseTip);
                        $(this).find(' > span').attr('title', options.i18n.collapseTip).find(' > span').addClass('glyphicon-minus-sign');
                    }
                    else {
                        //$(this).find(' > span > span').addClass('glyphicon-folder-close').attr('title', options.i18n.expandTip);
                        $(this).find(' > span').attr('title', options.i18n.expandTip).find(' > span').addClass('glyphicon glyphicon-plus-sign');
                    }
                    ...
                    if (link) {
                        $(this).find(' > span > a').attr('href', link);
                    }
                    ...
                    //$(this).html('<span><span class="glyphicon"></span>' + link + '</span>');
            ...
            // addable
            if (options.addable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="create"><button class="btn btn-default btn-sm btn-success"><span class="glyphicon glyphicon-plus-sign"></span></button></div> ');
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        if (selected.length <= 0) {
                            ...
                            if ($(selected).hasClass('parent_li')) {
                                ...
                                $(selected).addClass('parent_li').find(' > span > span').addClass('glyphicon-minus-sign').removeClass('glyphicon-file');
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content .confirm').on('click', function () {
                            ...
                            if($(selected).parent('ul').find(' > li').length <= 1) {
                                $(selected).parents('li').removeClass('parent_li').find(' > span > span').removeClass('glyphicon-minus-sign').addClass('glyphicon-file');
